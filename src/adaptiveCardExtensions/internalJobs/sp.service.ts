...
export const fetchListTitle = async (spContext: AdaptiveCardExtensionContext, listId: string): Promise<string> => {
  ...
  const response = await (await spContext.spHttpClient.get(
    `${spContext.pageContext.web.absoluteUrl}/sites/JobPostings/_api/web/lists/GetById(id='${listId}')/?$select=Title`,
...
export const fetchListItems = async (spContext: AdaptiveCardExtensionContext, listId: string): Promise<IListItem[]> => {
  ...
  const response = await (await spContext.spHttpClient.get(
    `${spContext.pageContext.web.absoluteUrl}/sites/JobPostings/_api/web/lists/GetById(id='${listId}')/items?$select=ID,Title,JobDescription,LinktoDescription`,
    SPHttpClient.configurations.v1
  )).json();
    ...
    return Promise.resolve(response.value.map(
      (listItem: any, index: number) => {
        return <IListItem>{
          ...
          description: listItem.JobDescription,
          index: index,
          jobUrl: listItem.LinktoDescription.Description
        };
      }
    ));
  } else {
...
export const fetchListLength = async (spContext: AdaptiveCardExtensionContext, listId: string): Promise<number> => {
  const response = await (await spContext.spHttpClient.get(
    `${spContext.pageContext.web.absoluteUrl}/sites/JobPostings/_api/web/lists/GetById(id='${listId}')/items?$select=ID`,
    SPHttpClient.configurations.v1
  )).json();
